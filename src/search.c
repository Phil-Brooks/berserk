...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss)
{
    ...
    while((move = NextMove(&mp, board, skipQuiets))) {
        ...
        if(score > bestScore) {
            ...
            // we're failing high
            if(alpha >= beta) {
                UpdateHistories(board, ss, thread, move, depth + (bestScore > beta + 100), board->stm, quiets,
                                numQuiets, captures, numCaptures, oppThreat.sqs);
                break;
            }
        }
    }
...
void PrintUCI(ThreadData* thread, int alpha, int beta, Board* board)
{
    ...
    for(int i = 0; i < Limits.multiPV; i++) {
        ...
        int printable = bounded > MATE_BOUND             ? (CHECKMATE - bounded + 1) / 2
                        ...
                        : abs(bounded) > WINNING_ENDGAME ? bounded
                                                         : // don't normalize our fake tb scores or real tb scores
        ...
        printf("info depth %d seldepth %d multipv %d score %s %d%s", realDepth, thread->rootMoves[i].seldepth, i + 1,
               type, printable, bound);
        ...
        printf("nodes %" PRId64 " nps %" PRId64 " hashfull %d tbhits %" PRId64 " time %" PRId64 " pv ", nodes, nps,
               hashfull, tbhits, time);
