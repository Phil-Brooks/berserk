...
inline void TTUpdate() { TT.age += AGE_INC; }

INLINE uint64_t TTIdx(uint64_t hash) { return ((unsigned __int128)hash * (unsigned __int128)TT.count) >> 64; }

inline void TTPrefetch(uint64_t hash) { __builtin_prefetch(&TT.buckets[TTIdx(hash)]); }
...
inline void TTPut(TTEntry* tt, uint64_t hash, int depth, int16_t score, uint8_t bound, Move move, int ply, int16_t eval,
                  int pv)
{
    uint16_t shortHash = (uint16_t)hash;
