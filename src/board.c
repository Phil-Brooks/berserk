...
int IsPseudoLegal(Move move, Board* board)
{
    ...
    if(Promo(move)) {
        ...
        BitBoard opts = (ShiftPawnDir(Bit(from), board->stm) & ~OccBB(BOTH)) |
                        (GetPawnAttacks(from, board->stm) & OccBB(board->xstm));
...
INLINE uint64_t Hash1(uint64_t hash) { return hash & 0x1fff; }

INLINE uint64_t Hash2(uint64_t hash) { return (hash >> 16) & 0x1fff; }
